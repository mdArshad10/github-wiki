...
export class RepoController {
	...
	readonly fetchAllRepos = asyncHandler(async (req, res) => {
		const userId = req.user.id;
		const accounts = await this.authRepo.api.listUserAccounts({
		...
		if (!githubAccount) {
			throw new AppError("GitHub account is not linked", 400);
		}

		const repos = await this.repoService.createAllRepo(
			userId,
			githubAccount.id,
		);
